...
def load_dataset_beta() -> TabminiDataset:
    """
    Load the dataset for AutoML. The datasets are loaded from the PMLB library. If get error, use data from original_data.
    :param reduced: Whether to exclude the datasets that have been used to train TabPFN. Default is False.
    :return: A dictionary containing the loaded dataset. The key is the dataset name and the value is a tuple containing
    the input features and the target variable.
    """
    ...
    for idx, _datasets in enumerate(data_info.files):
        ...
        for dataset_name in datasets:
            try:
                ...
                print(f"Dataset {dataset_name} is not available. Loading from original_data...")
                
                X_csv_path = f"{OFFLINE_DATA_DIR[dataset_name]}/X.csv"
                y_csv_path = f"{OFFLINE_DATA_DIR[dataset_name]}/y.csv"
